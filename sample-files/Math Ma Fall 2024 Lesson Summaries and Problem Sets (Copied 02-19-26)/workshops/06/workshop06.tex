...
\item  
  ...
  \item
    ...
    \begin{problem}[\nstretch{0.75}]
      Does $f(x)$ have a global maximum on $(-\infty, \infty)$? If so, where? How about a global minimum?
    \end{problem}
  ...
  \item
    ...
    \begin{workshop}
      Help students understand that the Extreme Value Theorem says: if a function $f$ is continuous on a closed interval $[a, b]$, then $f$ must have a global minimum and global maximum on $[a, b]$.

      If $f$ isn't continuous, or if you're looking on some domain other than a closed interval, then the Extreme Value Theorem gives no information. That is, $f$ \emph{could} have a global minimum or global maximum on the interval, but it doesn't \emph{have} to.      
    \end{workshop}

    \begin{problem}[\vfill\newpage]
      State the Extreme Value Theorem. Are your answers consistent with it? 
    \end{problem}
...
\item  \label{fall2023-workshop6-2}
  ...
  \item
    \begin{problem}[\nstretch{0.25}]
      Is this consistent with the Extreme Value Theorem? 
    \end{problem}
...
\item 
  \begin{workshop}
    This is practice with limits (since the skills check is coming up on Thursday).
  \end{workshop}

  \begin{problem}[\newpage]
    In \ref{fall2023-workshop6-2}, you found the discontinuities of $f(x) = \dfrac{x^3 + x^2 - 12 x}{x^2 - 16}$. Now, use limits to classify each discontinuity (as a removable discontinuity, jump discontinuity, or vertical asymptote).
  \end{problem}
