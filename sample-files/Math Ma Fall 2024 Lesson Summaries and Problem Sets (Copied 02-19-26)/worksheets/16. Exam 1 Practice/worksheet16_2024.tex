...
\item  \label{stretching-derivatives}
  ...
  \item \label{stretching-derivatives-vertical}
    ...
    \probonly{
      \begin{tikzpicture}
        \begin{axis}[xtick={-1, 2, 5}, ytick={10}, clip=false,
    ...
    \solonly{
          \begin{tikzpicture}
        \begin{axis}[xtick={-1, 2, 5}, ytick={10,20}, clip=false,
        width=2.75in, restrict y to domain=-30:30]
          \addplot+[domain=-2:6] {(x + 1)*(x-2)*(x-5)} node[right]{$y = f(x)$};
  ...
  \item
    ...
    \begin{problem}
      ...
      \solonly{
      \begin{tikzpicture}
        \begin{axis}[xtick={-1, 2, 5, -0.5,1,2.5},
        ytick={10}, clip=false, width=2.75in, 
        xticklabels={$-1$,$2$,$5$,$\frac{-1}{2}$,$1$,
        $\frac{5}{2}$}]
          \addplot+[domain=-2:6] {(x + 1)*(x-2)*(x-5)} node[right]{$y = f(x)$};
...
\item 
  ...
  \begin{enumerate}
      \item $\displaystyle \lim_{x\to2} \frac{x^2+x-6}{x+3}$
      %%%%%%%%%%%NEEDS SOLUTION
      \vfill 
