...
\item  \label{graph-cubic-with-repeated-root-followup}
  ...
  \begin{problem}[\newpage]
    In your last assignment, you found that the graph of $f(x) = (x-1)(x+3)^2 = x^3 + 5x^2 + 3x - 9$ looks like the picture below. If $d$ is a constant, how does the graph of $g(x) = x^3 + 5x^2 + 3x + d$ relate to the graph of $f$? How many roots could $g$ have?
...
\item \emph{Comparing quadratics and cubics.} Understanding the possible ways a particular type of function can behave is important for modeling and broader reasoning. Here we'll look at characteristics across quadratics and cubics before generalizing to higher degree polynomials.
  ...
  \item
    ...
    \begin{problem}[\vfill]
      If $f(x)$ is a quadratic, what can you say about $\displaystyle \lim_{x \to \infty} f(x)$ and $\displaystyle \lim_{x \to -\infty} f(x)$? (We call this the ``end behavior'' of $f$.) What if $f(x)$ is a cubic? Explain.
    \end{problem}
  ...
  \item
    ...
    \begin{problem}[\vfill]
      How many roots ($x$-intercepts) can a quadratic have? (Give all the possibilities.) How many roots can a cubic have? Explain.
    \end{problem}
  ...
  \item
    ...
    \begin{problem}[\vfill]
      How many turning points can a quadratic have? (Again, give all the possibilities.) How many turning points can a cubic have? Explain.
    \end{problem}
...
\item  Now we'll attempt to generalize! Suppose $f(x)$ is a degree $n$ polynomial.
  \begin{enumerate}
