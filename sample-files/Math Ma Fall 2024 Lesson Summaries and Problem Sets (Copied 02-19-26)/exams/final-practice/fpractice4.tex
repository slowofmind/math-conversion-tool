...
\item \label{fall2013-ma-premidterm-3} \points{12} 
  ...
  \item
    ...
    \begin{solution}
      ...
      \answer{\includegraphics[width=4in]{images/fall2013-ma-premidterm-3a}}
    \end{solution}    
...
\item \points{9} 
  ...
  \item % added 2023
    ...
    \begin{problem}[\vfill\newpage]
      If $f(x) = \dfrac{\sqrt[3]{e^{2x}} \ln(5x)}{e^x}$, find $f'(x)$. 
    \end{problem}

    \begin{solution}
      We need to rewrite $f(x)$ to get it in a form we can differentiate:
      \begin{align*}
        f(x) &= \dfrac{\sqrt[3]{e^{2x}} \ln(5x)}{e^x} \\
        &= \dfrac{e^{2x / 3} \ln(5x)}{e^x} \\
        &= e^{-x/3} \ln(5x) \\
        &= e^{-\frac{1}{3} x} (\ln 5 + \ln x) \\
        \intertext{Using the Product Rule,}
        f'(x) &= \boxed{- \frac{1}{3} e^{-\frac{1}{3} x} (\ln 5 + \ln x) + e^{- \frac{1}{3} x} \frac{1}{x}}
      \end{align*}
    \end{solution}
  \end{enumerate}
