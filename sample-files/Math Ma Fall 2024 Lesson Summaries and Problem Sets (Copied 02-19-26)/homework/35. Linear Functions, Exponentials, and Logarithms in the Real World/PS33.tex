...
\begin{center}
  \large\textsc{Problem Set 33 - Linear Functions, Exponentials, and Logarithms in the Real World}
\end{center}
...
\item \label{mouse-to-elephant-formula} 
  ...
  \item
    ...
    \begin{problem}[0.25in]
      ...
      \begin{tikzpicture}[x=1.4cm, y=1.57cm]
        \draw (0, 0) node[anchor=south west] (image) { \includegraphics[trim=0.46in 0.3in 0 0, clip]{images/systems-02-00089-g001.png}}; %
  ...
  \item
    ...
    \begin{problem}[\newpage]
      If a log-log plot of a function $f(x)$ is linear, which of the following describes the form that $f(x)$ has? (In each choice, $a$ and $b$ are constants.) 
...
\calcitem % based on 13.3.45
  ...
  \item
    ...
    \begin{problem}[\vfill]
      Suppose you deposit some money in a bank account that earns a nominal annual interest rate of 3\%, compounded monthly. (See Example 9.7 in \href{https://people.math.harvard.edu/~jjchen/mathma/docs/handouts/Gottlieb.html}{Gottlieb} or {{\#4}} on \href{https://people.math.harvard.edu/~jjchen/mathma/docs/worksheets/worksheet23-sols.html}{the worksheet ``Applications of Exponentials''} if you need a reminder of what this means.)
...
\end{enumerate}
\psreflection{For next class, read Chapter 6.}
\end{document}
