...
\section{{Inverses, $\vec\S$2.2}}% 
\chead{\Large\textbf{Inverses, $\vec\S$2.2}}% 
...
Multiplying the first equation by $c$ and the third equation by $-a$ and adding them together yields: $bcg-adg=c$, and assuming $ad-bc\neq0$, we get $g=\frac{-c}{ad-bc}$.
...
\end{enumerate}
  ...
  Since elementary matrices are invertible, we can rearrange $E_p E_{p-1}\dots E_2 E_1 A=I_n$ to get $$A=E_1^{-1} \dots E_p^{-1}.$$ Using the properties of inverses, we then get $$A=(E_1 \dots E_p)^{-1}.$$ Since $A$ is the inverse of a matrix, we conclude (from the properties of inverses) that $A$ is invertible and $A^{-1}=E_p\dots E_2 E_1$. Thus, if we apply the same sequence of row operations that transforms $A$ to $I_n$ to the identity matrix we get $E_p\dots E_2 E_1 I_n=A^{-1}$.
  \end{proof}
