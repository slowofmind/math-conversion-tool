...
**Related conversion findings (chunkedhtml, from the include-test experiments):** `-t chunkedhtml --split-level=2` gives index + chapter pages + one page per worksheet with prev/next/up nav and cross-chunk `\ref` link rewriting. Chunk filenames default to `%s-%i.html` where `%i` is the heading identifier — labels containing colons (`sec:foo`) produce Windows-illegal filenames that break every link after zip extraction; use `--chunk-template=section-%s.html` (or `%n.html`). Equation labels (`\label` inside math) are NOT resolved across chunks — refs render as `[eq:...]`; grep the real workbook for cross-worksheet equation refs before worrying. Known content-level issues still open for the real workbook: `\ifSubfilesClassLoaded{...}{...}` argument leakage, `workbook.sty` commands (specialsty/cleanup-tool territory).
