...
\providecommand{\probonly}[1]{#1}
\providecommand{\solonly}[1]{}
\providecommand{\spaceonly}[1]{#1}
\providecommand{\answeronly}[1]{}
\providecommand{\answer}[1]{#1}
\providecommand{\note}[1]{}
\providecommand{\htitle}[1]{\section*{#1}}
\providecommand{\stitle}[1]{}

\providecommand{\fitb}[3][\ ]{\underline{\hspace{#2}}}
\providecommand{\cfitb}[3][\ ]{\underline{\hspace{#2}}}
\providecommand{\mfitb}[3][\ ]{\underline{\hspace{#2}}}

\providecommand{\maybecircle}[2][]{#2}
\providecommand{\maybebubble}[1][]{$\circ$}
\providecommand{\circleitem}{\item}
\providecommand{\plainitem}{\item}

\providecommand{\continueproblem}[1]{\begin{framed}#1\end{framed}}
\providecommand{\psreflection}[2][]{}
\providecommand{\psrnewpage}{}

\providecommand{\shortintertext}[1]{\text{#1}}
...
\section{Continuity \& Reasoning About Signs} \label{ws3-3}
...
Over the course of the first few problems, we'll be filling in information in the following box. It's full of important definitions and properties!
\begin{framed}
  \textbf{Limit definitions of continuity / discontinuities}.
  \begin{enumerate}
...
\item 
  ...
  \note{Students have basically done this already in \href{https://people.math.harvard.edu/~jjchen/mathma/docs/homework/PS14.html}{Problem Set 14}, {{\#5}}, although that was worded differently. When we discuss this, the first thing I want students to recognize is that the difference between the functions in (a) and (b) is whether they're continuous at $a$. Then, we'll dive into types of discontinuities.}
...
\begin{framed}
 \textbf{A useful fact.} Any function we can write down with a single (non-piecewise) algebraic formula is continuous at every point in its domain. In other words, the discontinuities of a function described by a single formula are where it's undefined.
 \end{framed}
\begin{enumerate}[resume]
\item 
  \begin{problem}[\vfill]
    Let $f(x) = \dfrac{x - 3}{x - 2}$. Find all values of $x$ in $(-\infty, \infty)$ at which $f$ is discontinuous (by determining where it is not defined), and use limits to classify each (that is, to decide whether it's removable, a jump, or a vertical asymptote). 
  \end{problem}
...
\item  \label{continuity-differentiability-relationship}
  ...
  \textbf{Important implications!} Decide whether each of the following statements is true or false. If it's false, give an example that shows that it's false (such an example is known as a \textit{counterexample}). 
  \begin{enumerate}
  \item
    \begin{problem}[1in]
...
\item  \textbf{Summarize.} When we are working with continuous functions, which happens throughout calculus, some special properties hold, which we will frequently take advantage of. 
...
\end{enumerate}
\begin{framed}
    This is a specific application of a broader theorem, known as the \textbf{Intermediate Value Theorem}, which states that if some function $f$ is continuous on the closed interval $[a, b]$ and $f(a) = A$ and $f(b)=B$, then somewhere on the interval, $f$ attains every value between $A$ and $B$. (Gottlieb, p.271)
\end{framed}
\begin{enumerate}[resume]
  \item
    ...
    \begin{problem}[0.15in]
     \textbf{Key Skill:} There are many situations  where we want to determine the sign of a function.  If you want to make a \textbf{sign chart} for $g(x)$ and $g(x)$ is given by a single (non-piecewise) algebraic formula, you should start by finding values of $x$ where $g(x)$ is \fitb[$\vphantom{\displaystyle \dfrac{1}{1}}$]{2.5in}{0 or undefined}. 
    \end{problem}
...
\item 
  ...
  \begin{problem}[\vfill]
    \textbf{Practice using sign charts.} Suppose $f$ is a function with domain $(-\infty, \infty)$, and $f'(x) = \dfrac{x + 2}{\sqrt[3]{x}}$. Where is $f$ decreasing? Build a sign chart to determine the answer.
  \end{problem}
...
\item 
  ...
  \begin{problem}[\vfill\newpage]
    \textbf{Practice identifying and classifying discontinuities.} Let $f(x) = \dfrac{x^2 + 2x}{|x|}$. Find all values of $x$ at which $f(x)$ is discontinuous. For each such value $a$, calculate $\displaystyle \lim_{x \to a^+} f(x)$ and $\displaystyle \lim_{x \to a^-} f(x)$, and use this to classify the discontinuity $a$. 
  \end{problem}
